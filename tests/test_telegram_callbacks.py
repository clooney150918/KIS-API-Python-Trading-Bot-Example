...
def _callbacks(config=None):
    ...
    cb.queue_handler.handle = AsyncMock(side_effect=AssertionError("old queue handler called"))
    cb.avwap_handler.handle = AsyncMock(side_effect=AssertionError("old avwap handler called"))
...
@pytest.mark.parametrize("data,handler_name", [
    ...
    ("QUEUE:VIEW:SOXL", "queue_handler"),
    ("DEL_REQ:SOXL:2026-08-12", "queue_handler"),
    ("DEL_Q:SOXL:2026-08-12", "queue_handler"),
    ("EDIT_Q:SOXL:2026-08-12", "queue_handler"),
    ("AVWAP:MENU:SOXL", "avwap_handler"),
    ("AVWAP_SET:FOO:SOXL", "avwap_handler"),
    ("MODE:AVWAP_ON:SOXL", "avwap_handler"),
    ("SET_VER_CONFIRM:V14_VWAP:SOXL", "config_handler"),
])
