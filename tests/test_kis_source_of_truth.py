...
class _AppendOnlyVRevCfg(_AppendOnlySyncCfg):
    def __init__(self):
        super().__init__()
        self.FILES = {"HISTORY": "forbidden_history.json"}
        self.legacy_reads = []
    ...
    def calculate_holdings(self, ticker, rows):
        # Match the V-REV queue-derived avg to reach the V-REV sync branch;
        # the regression is specifically about queue/history/snapshot mutation there.
    ...
    def set_seed(self, *args, **kwargs):
        return self._legacy_mutation("set_seed")

    def _load_json(self, *args, **kwargs):
        self.legacy_reads.append(("_load_json", args))
        return []

    def _save_json(self, *args, **kwargs):
        return self._legacy_mutation("_save_json")

    def get_active_tickers(self):
        return ["SOXL"]


class _AppendOnlyVRevBroker:
    def get_recent_stock_split(self, ticker, last_split_date):
        return 0.0, ""

    def get_account_balance(self):
        return 1000.0, {"SOXL": {"qty": 0, "avg": 0.0}}
...
class _AppendOnlyVRevQueueLedger:
    def __init__(self):
        self.calls = []

    def get_queue(self, ticker):
        return [{"date": "2026-08-12 09:30:00", "qty": 2, "price": 100.0}]

    def overwrite_queue(self, *args, **kwargs):
        self.calls.append(("overwrite_queue", args, kwargs))
        return True

    def sync_with_broker(self, *args, **kwargs):
        self.calls.append(("sync_with_broker", args, kwargs))
        return True


class _AppendOnlyVRevStrategy:
    def capture_vrev_snapshot(self, *args, **kwargs):
        return {
            "captured_at": "2026-08-12",
            "realized_pnl": 21.0,
            "realized_pnl_pct": 10.5,
            "avg_price": 100.0,
            "cleared_qty": 2,
            "clear_price": 110.5,
        }
...
class _AppendOnlyVRevView:
    def create_profit_image(self, *args, **kwargs):
        return None
...
async def _async_send_message(**kwargs):
    return SimpleNamespace(**kwargs)


def test_process_auto_sync_vrev_blocks_queue_history_seed_snapshot_mutation_but_appends_official_facts(monkeypatch):
    cfg = _AppendOnlyVRevCfg()
    queue_ledger = _AppendOnlyVRevQueueLedger()
    removed_paths = []
    sent_messages = []

    async def capture_send_message(**kwargs):
    ...
    monkeypatch.setattr(telegram_sync_engine.GlobalThrottle, "wait_api_sync", lambda: None)
    monkeypatch.setattr(telegram_sync_engine.asyncio, "sleep", _async_noop)
    monkeypatch.setattr(telegram_sync_engine.glob, "glob", lambda pattern: [pattern.replace("*", "sample")])
    monkeypatch.setattr(telegram_sync_engine.os, "remove", lambda path: removed_paths.append(path))

    engine = TelegramSyncEngine(
        cfg,
        broker=_AppendOnlyVRevBroker(),
        strategy=_AppendOnlyVRevStrategy(),
        queue_ledger=queue_ledger,
        view=_AppendOnlyVRevView(),
        tx_lock=asyncio.Lock(),
        sync_locks={},
    )
    context = SimpleNamespace(bot=SimpleNamespace(send_message=capture_send_message))

    result = asyncio.run(engine.process_auto_sync("SOXL", 12345, context))

    assert "V-REV" in result
    assert "append-only" in result
    assert cfg.legacy_calls == []
    assert cfg.legacy_reads == []
    assert queue_ledger.calls == []
    assert removed_paths == []
    assert len(cfg.appended) == 1
    assert cfg.appended[0]["source"] == "KIS_CONFIRMED_FILL"
    assert cfg.appended[0]["side"] == "SELL"
    assert cfg.appended[0]["qty"] == 2
    assert cfg.appended[0]["kis_order_no"] == "VREV-SELL-1"
    assert any("V-REV" in message and "append" in message for message in sent_messages)


class _AppendOnlyVRevPositiveBroker:
    def get_recent_stock_split(self, ticker, last_split_date):
        return 0.0, ""

    def get_account_balance(self):
        # positive holding reaches the  datetime.datetime.now  branch
        return 1000.0, {"SOXL": {"qty": 98, "avg": 158.0735}}
    ...
    def get_execution_history(self, ticker, start, end):
        raise AssertionError("should prefer get_execution_fills")
...
class _AppendOnlyVRevPositiveCfg(_AppendOnlyVRevCfg):
    def _save_json(self, path, *args, **kwargs):
        # allow kis_balance.json save, but block any HISTORY mutation
        if isinstance(path, str) and "kis_balance" in path:
            return
        return self._legacy_mutation("_save_json")

    def calculate_holdings(self, ticker, rows):
        # match the broker's positive holding to avoid diff != 0 reconstruction branch
        # and match V_REV queue-derived avg to skip price_diff guard
        return (98, 100.0, 0.0, 0.0)


def test_process_auto_sync_vrev_positive_holding_does_not_raise_datetime_now_attribute_error_and_appends_official_facts(monkeypatch):
    """Regression: datetime.now → datetime.datetime.now fix for positive V_REV holdings."""
    cfg = _AppendOnlyVRevPositiveCfg()
    queue_ledger = _AppendOnlyVRevQueueLedger()
    removed_paths = []
    sent_messages = []
    ...
    engine = TelegramSyncEngine(
        cfg,
        broker=_AppendOnlyVRevPositiveBroker(),
        strategy=_AppendOnlyVRevStrategy(),
        queue_ledger=queue_ledger,
        view=_AppendOnlyVRevView(),
        tx_lock=asyncio.Lock(),
    ...
    assert cfg.appended[0]["source"] == "KIS_CONFIRMED_FILL"
    assert cfg.appended[0]["side"] == "SELL"
    assert cfg.appended[0]["qty"] == 5
    assert cfg.appended[0]["kis_order_no"] == "VREV-POSFILL-1"
