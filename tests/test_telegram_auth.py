...
@pytest.mark.parametrize("method_name,args", [
    ("cmd_queue", ["SOXL"]),
    ("cmd_add_q", ["SOXL", "2026-08-12", "1", "10.0"]),
    ("cmd_clear_q", ["SOXL"]),
    ("cmd_update", []),
    ("cmd_avwap", ["SOXL"]),
])
...
def test_official_command_menu_excludes_error_and_blocked_commands():
    ...
    for allowed in ["start", "record", "history", "sync", "settlement", "seed", "ticker", "mode", "reset", "version", "log"]:
...
@pytest.mark.parametrize("renderer,args", [
    ("get_queue_management_menu", ("SOXL", [{"qty": 1, "price": 10, "date": "2026-08-12"}])),
    ("get_reset_menu", (["SOXL"],)),
