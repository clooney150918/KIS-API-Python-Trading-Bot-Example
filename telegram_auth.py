...
_ALLOWED_COMMANDS: tuple[tuple[str, str], ...] = (
    ...
    ("settlement", "설정 조회"),
    ("seed", "시드 설정"),
    ("ticker", "SOXL 운용 종목 상태"),
    ("mode", "HALT/SHADOW/LIVE 안전상태"),
...
def is_blocked_callback_action(action: str, sub: str = "") -> bool:
    if action in BLOCKED_CALLBACK_ACTIONS:
        return True
    if action == "MODE" and str(sub).upper().startswith(("AVWAP", "VWAP", "SNIPER")):
        return True
    if action == "SET_VER_CONFIRM" and any(token in str(sub).upper() for token in ("VWAP", "V_REV", "SNIPER", "AVWAP")):
        return True
    if action == "CONFIG_AVWAP":
        return True
    if action == "INPUT" and any(token in str(sub).upper() for token in ("AVWAP", "VWAP", "SNIPER")):
        return True
    return False
