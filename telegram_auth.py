...
def is_blocked_callback_action(action: str, sub: str = "") -> bool:
    if action in BLOCKED_CALLBACK_ACTIONS:
        return True
    if action == "MODE" and str(sub).upper().startswith(("SLICE", "VOLATILITY")):
        return True
    if action == "SET_VER_CONFIRM" and any(token in str(sub).upper() for token in ("SLICE", "VOLATILITY")):
        return True
    if action == "INPUT" and any(token in str(sub).upper() for token in ("SLICE", "VOLATILITY")):
        return True
    return False
