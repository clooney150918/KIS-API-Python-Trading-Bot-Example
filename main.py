...
from daily_report import scheduled_daily_report
...
def main():
    ...
    app_data = {
        ...
        'tz_est': est_zone, 'admin_chat_id': ADMIN_CHAT_ID,
        'regime_data': {"status": "pending", "msg": "10:00 EST 이전 오프닝 휩소 대기"} 
    }
    ...
    for cmd, handler in [
        ...
        ("log", bot.cmd_log), ("error", bot.cmd_log), ("report", bot.cmd_report)
    ]:
    ...
    jq.run_daily(scheduled_daily_report, time=datetime.time(19, 0, tzinfo=est_zone), days=tuple(range(7)), chat_id=ADMIN_CHAT_ID, data=app_data)
