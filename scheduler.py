...
async def scheduled_self_cleaning(context):
    ...
    logging.info("🧹 [시스템 자정 작업 완료] 7일 초과 로그/백업 및 24시간 초과 임시 파일 정리 완료")
...
async def scheduled_volatility_monitor(context):
            ...
            for t in cfg.get_active_tickers():
                            ...
                            if candle:
                                ...
                                if not tracking_info['is_tracking'] and current_drop >= (abs(volatility_pct) / 100.0):
                                    ...
                                    if not tracking_info['alerted']:
                                        msg = f"🎯 <b>[{t}] 하방 매수 변동성 안전장 해제 (Armed)!</b>\n"
                                ...
                                if tracking_info['is_tracking']:
                                    ...
                                    if is_yangbong and is_rebounded and is_volume_spike:
                                        is_deep_oversold = True if c_slice <= 0 else (c_close < c_slice)
                                        ...
                                        if is_deep_oversold:
                                            if cfg.get_secret_mode():
                                                ...
                                                if volatility_budget >= exec_price and exec_price > 0:
                                                    ...
                                                    if hunt_success:
                                                        ...
                                                        msg = f"💥 <b>[{t}] 하방(매수) 변동성 명중! ({session_str})</b>\n"
                                                        ...
                                                        msg += "🔫 당일 하방 변동성 활동만을 종료하며, 상방(익절) 감시는 계속됩니다."
                                                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
                                                        continue
                                                    ...
                                                    if now_ts - fail_history.get(t, 0) > 60:
                                                        msg = f"🛡️ <b>[{t}] 능동형 변동성 기습 실패 (1분 쿨타임 진입)</b>\n"
                                                        msg += f"📉 반등 타점(${exec_price:.2f})에 3회 지정가 덫을 던졌으나 잔량이 남았습니다.\n"
                                                        msg += f"🦇 매수 변동성는 1분 동안 숨을 죽이며, 취소했던 방어 매수(LOC) 주문만 호가창에 정밀 복구합니다."
                                                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
                                            ...
                                            now_ts = time.time()
                                            fail_history = app_data.setdefault('volatility_fail_ts', {})
                                            if now_ts - fail_history.get(f"{t}_slice", 0) > 300:
                                                msg = f"🛡️ <b>[{t}] 하방 변동성 매수 취소 (SLICE 필터 가동)</b>\n"
                                                msg += f"반등 조건은 충족했으나 현재가(${c_close:.2f})가 당일 평균가(${c_slice:.2f})를 넘어선 비싼 가격입니다. 가짜 반등(데드캣 바운스)으로 판별하여 사격을 취소합니다."
                                                await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
                                                fail_history[f"{t}_slice"] = now_ts
                    else:
                        pass
                ...
                if target_pct_val := cfg.get_target_profit(t):
                    ...
                    if not lock_sell and curr_p >= target_price:
                        ...
                        if now_ts - fail_history_j.get(t, 0) > 60:
                            msg = f"🛡️ <b>[{t}] 변동성 잭팟 기습 실패 (방어선 복구)</b>\n"
                ...
                if not lock_sell and curr_p >= base_trigger:
                    if not tracking_info['trailing_armed']:
                        ...
                        msg = f"🦇 <b>[{t}] 상방 트레일링 변동성 락온 (Armed)!</b>\n\n"
                ...
                if not lock_sell and tracking_info['trailing_armed']:
                    ...
                    candle = await asyncio.to_thread(broker.get_current_5min_candle, t)
                    c_slice = candle.get('slice', 0.0) if candle else 0.0
                    is_slice_death_cross = (c_slice > 0 and curr_p < c_slice)
                    
                    if curr_p <= drop_trigger or is_slice_death_cross:
                        intercept_price = max(drop_trigger, base_trigger)
                        
                        if is_slice_death_cross and curr_p > drop_trigger:
                        ...
                        if target_odno:
                            ...
                            if now_ts - fail_history_sync.get(t, 0) > 300:
                                msg = f"⚠️ <b>[{t}] 상방 변동성 가로채기 대기 (이중 체결 방지)</b>\n"
                        ...
                        if hunt_success:
                            ...
                            msg = f"💥 <b>[{t}] 상방 쿼터 변동성 명중! (기습 익절 완료)</b>\n\n"
                            msg += f"▫️ 최고점 갱신: ${tracking_info['peak_price']:.2f}\n"
                            
                            if is_slice_death_cross and curr_p > drop_trigger:
                                msg += f"▫️ 지지선 붕괴: 당일 SLICE(${c_slice:.2f}) 데드크로스 포착 (조기 기습 익절 가동)\n\n"
                            else:
                            ...
                            if not is_rev:
                                if t_val < (split / 2):
                                    ...
                                    msg += "\n🛡️ <b>[공수 완벽 분리]</b> 후반전 하방 보너스 덫은 그대로 유지합니다."
                        ...
                        if now_ts - fail_history_q.get(t, 0) > 60:
                            msg = f"🛡️ <b>[{t}] 변동성 쿼터 가로채기 실패 (방어선 복구)</b>\n"
    ...
    try:
        ...
        if now_ts - fail_history_t > 1800: 
            await context.bot.send_message(chat_id=context.job.chat_id, text="🚨 <b>[긴급] 야후 파이낸스 등 외부 API 응답 지연으로 변동성 감시가 무한 대기 상태에 빠졌습니다. \n\n1회 건너뛰고 통제권(Lock)을 강제 반환하여 정규장 로직을 보호합니다!</b>", parse_mode='HTML')
            app_data['volatility_timeout_ts'] = now_ts
    except Exception as e:
        logging.error(f"🚨 변동성 모니터 에러: {e}")
...
async def scheduled_slice_trade(context):
    ...
    app_data = context.job.data
    cfg, broker, slice_strategy, strategy, tx_lock = app_data['cfg'], app_data['broker'], app_data['slice_strategy'], app_data['strategy'], app_data['tx_lock']
    chat_id = context.job.chat_id
    ...
    async def _do_slice():
        async with tx_lock:
            ...
            for t in cfg.get_active_tickers():
                ...
                # 💡 오리지널 정규장 예산/수량 산출 (SLICE 엔진에 주입할 타겟 목표량 추출용)
                ...
                target_buy_budget = 0.0
                target_sell_qty = 0
                star_price = plan.get('star_price', 0.0)
                
                for o in plan.get('core_orders', []):
                    if o['side'] == 'BUY':
                        target_buy_budget += (o['qty'] * o['price'])
                    elif o['side'] == 'SELL' and o['type'] == 'LOC': 
                        target_sell_qty += o['qty']
                
                # 💡 15:30 EST 정각: 핀셋 철거 (Targeted Cancellation)
                if not slice_cache.get(f"{t}_cancelled"):
                    await context.bot.send_message(chat_id=chat_id, text=f"🚨 <b>[{t}] SLICE 모드 락온: 12% 잭팟/보너스 방어선 보존 및 공수 핀셋 철거 (15:30 EST)</b>", parse_mode='HTML')
                    ...
                    slice_cache[f"{t}_cancelled"] = True
                    await asyncio.sleep(5.0) 
                    cash, holdings = broker.get_account_balance()
                
                # 💡 매수(BUY) SLICE 타임 슬라이싱
                executed_buy_budget = slice_cache.get(f"{t}_buy_executed", 0.0)
                remaining_buy_budget = max(0.0, target_buy_budget - executed_buy_budget)
                
                if remaining_buy_budget > 0:
                    slice_buy_plan = slice_strategy.get_slice_plan(t, curr_p, remaining_buy_budget, side="BUY")
                    
                    if slice_buy_plan['orders']:
                        for o in slice_buy_plan['orders']:
                            # 💡 1호가 타격: 매수 시 '매도 1호가' 스캔
                            ask_price = await asyncio.to_thread(broker.get_ask_price, t)
                            exec_price = ask_price if ask_price > 0 else o['price']
                            
                            res = broker.send_order(t, o['side'], o['qty'], exec_price, o['type'])
                            if res.get('rt_cd') == '0':
                                slice_cache[f"{t}_buy_executed"] = executed_buy_budget + (o['qty'] * exec_price)
                            await asyncio.sleep(0.2)
                            
                # 💡 매도(SELL) SLICE 타임 슬라이싱
                executed_sell_qty = slice_cache.get(f"{t}_sell_executed", 0)
                remaining_sell_qty = max(0, target_sell_qty - executed_sell_qty)
                
                if remaining_sell_qty > 0 and curr_p >= star_price:
                    slice_sell_plan = slice_strategy.get_slice_plan(t, curr_p, remaining_sell_qty, side="SELL")
                    
                    if slice_sell_plan['orders']:
                        for o in slice_sell_plan['orders']:
                            # 💡 1호가 타격: 매도 시 '매수 1호가' 스캔
                            bid_price = await asyncio.to_thread(broker.get_bid_price, t)
                            ...
                            if exec_price >= star_price: 
                                res = broker.send_order(t, o['side'], o['qty'], exec_price, o['type'])
                                if res.get('rt_cd') == '0':
                                    slice_cache[f"{t}_sell_executed"] = executed_sell_qty + o['qty']
                                await asyncio.sleep(0.2)
                        
    try:
        await asyncio.wait_for(_do_slice(), timeout=45.0)
    except Exception as e:
        logging.error(f"🚨 SLICE 스케줄러 에러: {e}")
    ...
    async def _do_regular_trade():
        async with tx_lock:
            ...
            for t in sorted_tickers:
                # 💡 [데드맨 스위치] V_SLICE 모드라도 V14와 동일하게 정규장 LOC 장전을 수행함 (Continue 삭제)
                ...
                if plan['orders']:
                    ...
                    ver_txt = "SLICE 장전" if cfg.get_version(t) == "V_SLICE" else "정규장 주문"
