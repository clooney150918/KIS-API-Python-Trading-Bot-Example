...
class TelegramView:
    ...
    def get_start_message(self, target_hour, season_icon, latest_version):
        ...
        msg += "💠 라오어 순정 무한매수법 (SOXL 전용) & 순수 돌파/추종 데이 트레이딩\n\n"
        ...
        msg += "▶️ /settlement : ⚙️ 설정/전술설정\n"
        msg += "▶️ /seed : 💵 개별 시드머니 관리\n"
        msg += "▶️ /ticker : 🔄 운용 종목 선택\n"
        msg += "▶️ /mode : 🎯 변동성 지표 분석\n"
    ...
    def get_reset_menu(self, active_tickers):
        ...
        msg += "⚠️ <b>장부 영구 소각 (삼위일체):</b> 본장부, 백업, 큐, 독립 데이터를 100% 영구 삭제합니다. HTS로 100% 수동 청산 후 새출발할 때만 격발하십시오.\n"
        msg += "⚠️ <b>독립 장부 초기화:</b> 수동 개입으로 어긋난 독립 장부 및 찌꺼기만 제거하고 본진은 안전하게 보존합니다.\n"
        ...
        for t in active_tickers:
            ...
            # NEW: 암살자 단독 소각 버튼 락온
            keyboard.append([
                 InlineKeyboardButton(f"🔫 {safe_t} 독립 장부 초기화", callback_data=f"RESET:AVWAP:{t}")
            ])
    ...
    def get_avwap_reset_confirm_menu(self, ticker):
        safe_t = html.escape(str(ticker))
        msg = f"🚨 <b>[{safe_t} 독립 장부 소각 최종 확인]</b>\n\n"
        ...
        keyboard = [
            [InlineKeyboardButton("🔥 네, 독립 정보만 소각합니다", callback_data=f"RESET:AVWAP_CONFIRM:{ticker}")],
            [InlineKeyboardButton("❌ 아니오, 취소합니다", callback_data="RESET:CANCEL")]
        ]
        return msg, InlineKeyboardMarkup(keyboard)
    ...
    def get_queue_management_menu(self, ticker, q_data):
        ...
        safe_t = html.escape(str(ticker))
        msg = f"🗄️ <b>[ {safe_t} 지층 큐(Queue) 정밀 관리 ]</b>\n\n"
    ...
    def get_settlement_message(self, active_tickers, config, atr_data, tracking_cache=None):
        ...
        for t in active_tickers:
            ...
            if ver == "V_REV":
                ...
                msg += f"▫️ 독립 타격망: <b>{avwap_status}</b>\n"
                
                if is_avwap_hybrid:
                    msg += f"▫️ 아키텍처: <b>본진(15%)과 독립 전략 병렬 가동</b>\n"
    ...
    def create_ledger_dashboard(self, ticker, qty, avg, invested, sold, records, t_val, split, is_history=False, is_reverse=False, history_id=None, assassin_data=None):
        ...
        if not is_history:
            ...
            keyboard.append([InlineKeyboardButton(f"🗄️ {safe_t} 큐 관리", callback_data=f"QUEUE:VIEW:{ticker}")])
