...
class NormalState:
    ...
    t: object
    reverse: bool = False
    target_profit_percent: object = Decimal("20")
...
def _target_sell_price(avg_price: Decimal, target_profit_percent: object) -> Decimal:
    pct = _parse_decimal(target_profit_percent)
    if pct is None or pct < ZERO:
        pct = TWENTY
    multiplier = ONE + (pct / Decimal("100"))
    return _round_cent(avg_price * multiplier) if avg_price > ZERO else ZERO
...
def _empty_normal_plan(
    ...
    *,
    ...
    target_sell_price = _target_sell_price(avg_price, state.target_profit_percent)
    return NormalPlan(
        ticker=state.ticker,
        split=state.split,
        t=t,
...
def apply_fill_event(t_before: Decimal, event: FillEvent) -> Decimal:
    ...
    if fill_event is FillEvent.BONUS_BUY:
        return t
